...
def amr_abm(t, agents_state, gamma, beta, alpha, movement, ward2size, model_settings):
    ...
    """
    agents_state = np.array(agents_state)
    ...
    #agents_state = agents_state - agents_state * (np.random.uniform(size=(Np, )) < α)
...
def observe_cluster(t, patients_state, movement, rho, model_settings):
    ...
    p_test            = Observed.yes * (np.random.random(size=(patients_state.shape[0], m)) <= patients_state * ρ)
